...
class PlayerAgent(object):
    def __init__(self, name, agentIndex, color, depth = 3, evalFn = defaultEvalFn):
        ...
        self.settlements = []
        self.cities = []
        self.resources = Counter({ResourceTypes.WOOL: 0, ResourceTypes.BRICK: 0, ResourceTypes.ORE: 0, ResourceTypes.GRAIN: 0, ResourceTypes.LUMBER: 0})
    ...
    def canSettle(self):
        return (self.resources[ResourceTypes.BRICK] >= 1 and
                self.resources[ResourceTypes.LUMBER] >= 1 and
                self.resources[ResourceTypes.WOOL] >= 1 and
                self.resources[ResourceTypes.GRAIN] >= 1)

    def canBuildCity(self):
        return self.resources[ResourceTypes.ORE] >= 3 and self.resources[ResourceTypes.GRAIN] >= 2
    ...
    def applyAction(self, action, board):
        ...
        if action[0] == ACTIONS.SETTLE:
            ...
            self.settlements = [s for s in self.settlements if not (s.X == vertex.X and s.Y == vertex.Y)]
...
def updateVictoryPoints(self):
    self.victoryPoints = (len(self.settlements) * SETTLEMENT_VICTORY_POINTS +
...
PlayerAgent.updateVictoryPoints = updateVictoryPoints
PlayerAgent.collectInitialResources = collectInitialResources